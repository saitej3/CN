...
int main()
{
	...
	char buffer[30];
	sprintf(buffer,"%d",8050);
	send(nfd,buffer,30,0); //send  newport to connect to
	...
	set = setsockopt(newsfd, SOL_SOCKET, SO_REUSEADDR, &(int){ 1 }, sizeof(int));
	error_func(set,"calling sockpot");
	error_func(bind(newsfd,(struct sockaddr*)&serv_addr,sizeof(serv_addr)),"bind");
	error_func(listen(newsfd,5),"listen");
	int newnfd=accept(newsfd,(struct sockaddr*)&client_addr,&len);
	error_func(newnfd,"accept");
	FILE *fp;
   	fp = fopen(buffer, "r+");
   	char buf[255];
   	while(!feof(fp))
